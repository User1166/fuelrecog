...
class WebcamDetector:
    ...
    def preprocess_frame(self, frame):
        ...
        # Resize maintaining aspect ratio would be better, but for speed:
...
def webcam_detect(weights=WEIGHTS_FILE):
    ...
    cap.set(cv2.CAP_PROP_FPS, 30)  # Try for 30 FPS
    
    print(f"Starting detection. Press 'q' to quit, 's' to save current frame.")
    ...
    try:
        while True:
            ...
            if key == ord('q'):
                ...
                cv2.imwrite(f"capture_{timestamp}.jpg", result_frame)
